...
# out:    mu - length C list of d x 1 vectors of class means
#      sigma - length C list of d x d matrices of class covariances
...
#         mu - length C list of d x 1 vectors of class means
#      sigma - length C list of d x d matrices of class covariances
...
def plotBoundary(dataset='iris',split=0.7,doboost=False,boostiter=5,covdiag=True):
    ...
    for (xi, xx) in enumerate(xRange):
        for (yi, yy) in enumerate(yRange):
            if doboost:
                ## Boosting 
                grid[yi,xi] = classifyBoost(np.matrix([[xx, yy]]),priors,mus,sigmas,alphas)        
            else:
                ## Simple
